...
public class LetterObjectPoolingManager : MonoBehaviour
{
        ...
        int row)
    {
        foreach (GameObject letter in letters)
        {
            if (!letter.activeInHierarchy)
            {
                ...
                letter.GetComponent<LetterBox>().SetSprites(
                    ...
                    correct_alphabet,
                    wrong_alphabet);
                if(letter.GetComponent<LetterBox>().Founded)
                {
                    letter.GetComponent<LetterBox>().SetCorrectSprite();
                }
                else
                {
        ...
        prefabInstance.GetComponent<LetterBox>().SetSprites(
            ...
            correct_alphabet,
            wrong_alphabet);
        if (prefabInstance.GetComponent<LetterBox>().Founded)
        {
            prefabInstance.GetComponent<LetterBox>().SetCorrectSprite();
        }
        else
        {
